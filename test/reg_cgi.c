...
int return_status(char *status_num)
{
    char *out;
...
void deal_reg_query(void)
{
    ...
    MYSQL *mysql_conn = msql_conn(MYSQL_USER, MYSQL_PWD,MYSQL_DATABASE);
    if (mysql_conn == NULL) {
        LOG(REG_LOG_MODULE, REG_LOG_PROC, "mysql connect error");
        return;
    }
    ...
    if (mysql_query(mysql_conn, buffer) != 0)  {
        //插入失败，原因nicheng或者u_name已经存在 
        LOG(REG_LOG_MODULE, REG_LOG_PROC, "mysql insert error");
        return_status("001");
        return;
    }
    ...
    rop_set_hash(redis_conn, "USER_USERID_HASH", user, user_id);
